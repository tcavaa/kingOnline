  ...
  return (
    <div
      ...
      style={{
        ...
        background: '#fffaf0',
        border: highlight ? '2.5px solid #8e2b23' : '1.5px solid rgba(120,70,30,0.45)',
        boxShadow: highlight
          ? '0 0 18px rgba(142,43,35,0.65), 0 4px 12px rgba(0,0,0,0.45)'
...
export default function LastTrickView({ onClose }) {
  ...
  const winnerName = players.find(p => p.seat === winnerSeat)?.name ?? `მოთამაშე ${winnerSeat}`
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center justify-between">
        ...
        {onClose && (
          <button onClick={onClose}
                  ...
                  style={{ color: 'rgba(59,35,20,0.7)' }}><X size={14} /></button>
        )}
      </div>
      ...
      <div className="rounded-xl p-3"
        ...
        <p className="text-base font-western mb-3" style={{ color: '#8e2b23' }}>{winnerName}</p>

        <div className="flex gap-2 justify-center flex-wrap">
          {trick.map(({ seat, card }, i) => {
            const playerName = players.find(p => p.seat === seat)?.name ?? `მ${seat}`
            return (
              <div key={i} className="flex flex-col items-center gap-1">
                <MiniCard card={card} highlight={seat === winnerSeat} />
                <span className="text-[10px] font-typewriter"
                      style={{ color: 'rgba(59,35,20,0.7)' }}>{playerName}</span>
              </div>
            )
          })}
        </div>
